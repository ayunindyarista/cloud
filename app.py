...
def list_tweets():
    ...
    for row in cursor:
        a_dict = {}
        a_dict['tweetedby'] = row[0]
        a_dict['body'] = row[1]
        a_dict['timestamp'] = row[2]
        a_dict['id'] = row[3]
        api_list.append(a_dict)
...
def add_tweet(new_tweets):
    ...
    api_list=[]
    cursor=conn.cursor()
    cursor.execute("SELECT * from tweets where username=?",(new_tweets['username'],))
    data = cursor.fetchall()
    if len(data) != 0:
        abort(409)
    else:
...
if __name__ == '__main__':
    app.run(host='127.0.0.01', port=5000, debug=True)
